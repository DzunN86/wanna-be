...
export const HIGHLIGHT_MODE = {
  [HIGHLIGHT_TYPE.ELEMENT]: (
    ...
    highlighterOptions.isHighlightGlyph &&
      decorators.push({
        range: HIGHLIGHT_MODE.locToMonacoRange(loc),
  ...
  [HIGHLIGHT_TYPE.EDGE]: (
    ...
    const loc = path.node.loc
    let innerLocKey = path.isJSXSpreadChild()
      ? 'expression'
      : path.isJSXSpreadAttribute()
      ? 'argument'
      : null
...
export const JSXTypes = {
  ...
  JSXElement: {
    ...
    options: (elementName) => ({
      ...
      glyphMarginHoverMessage: `JSX Element${
...
class MonacoJSXHighlighter {
  ...
  resetDeltaDecorations() {
    this.JSXDecoratorIds =
      this.monacoEditor &&
      this.monacoEditor.deltaDecorations(this.JSXDecoratorIds || [], [])
  }
  ...
  highLightOnDidChangeModelContent = (
    ...
    if (!monaco.jsxAutoComplete) {
      monaco.jsxAutoComplete = true
    }
  ...
  extractAllDecorators = (jsxManager = this.jsxManager) => {
    ...
    this.JSXDecoratorIds = this.monacoEditor.deltaDecorations(
      this.JSXDecoratorIds || [],
      decorators
    )
    return decorators
  ...
  getJSXContext = (
    ...
    let startColumn = monacoEditor
      .getModel()
      .getLineFirstNonWhitespaceColumn(selection.startLineNumber)
  ...
  runJSXCommentContextAndAction = (
    ...
    return new Promise((resolve) => {
      if (this._isHighlightBoundToModelContentChanges) {
        resolve(
          runJsxCommentAction(this.getJSXContext(selection, this.ast, editor))
        )
      } else {
        getAstPromise()
          .then((ast) => {
            resolve(
              runJsxCommentAction(this.getJSXContext(selection, ast, editor))
            )
          })
          .catch(
            (error) =>
              resolve(
                runJsxCommentAction(this.getJSXContext(selection, null, editor))
              ) || onJsCodeShiftErrors(error)
          )
      }
    }).catch(
      ...
      () => {
        ...
        const runJsxCommentAction = (commentContext) => {
          ...
          for (
            ...
            isUnCommentAction =
              isUnCommentAction && !!commentText.match(/{\/\*/)
          }
